...
def mock_external_services(monkeypatch):
    """
    Mock Kroger API + ShoppingCart so tests do not require
    .env secrets or real network calls.
    """
    ...
    mock_cart.get_cheapest_ingredients.return_value = {
        "milk": {
            "description": "Mock Milk",
            "price": 3.99
        }
    }

    # Patch objects inside the Dash app
    monkeypatch.setattr("app.app.KrogerStoreLocator", FakeStoreLocator)
    monkeypatch.setattr("app.app.ShoppingCart", lambda store_id: mock_cart)
...
class TestCookingHelper(unittest.TestCase):
    ...
    @given(st.text())
    def test_parse_r_list_fuzzing(self, s):
        """Ensure parser never crashes regardless of input."""
        result = parse_r_list(s)
        self.assertIsInstance(result, list)
